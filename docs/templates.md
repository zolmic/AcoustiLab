...
diaphragm therefore resonated on the air at 0.94 kHz with a Q near 5: the
drum level rose 13 dB above the bass to 122 dB at 1 kHz, the input
impedance peaked at 43.8 Ω there, and the response fell 12 dB per octave
above it. Published data on circumaural cups:
...
| In-situ impedance maximum | about 100 Hz for a circumaural headphone (about 6 kHz for an intra-concha one) | Fraunhofer, US 11,039,243 B2 (2016) | patent, measured example |
...
resonance 13 dB high at 1 kHz, with a 34 % impedance peak there, is not
what measured closed-back cups show: the one in-situ impedance quoted for a
circumaural headphone peaks near 100 Hz, and a common closed-back measures
flat. No public source gives the damping of a given product's driver. The
...
added, with its parameters (`damping_rayl`, 0 removes it;
`damping_area_cm2` and `driver_back_volume_cm3` in the detailed view). 600
...
| Without the damping cloth | 109.7 | 106.7 | 107.4 | 114.2 (peak 122.0 at 843 Hz) | 89.2 | 70.6 Ω at 843 Hz |
...
**Anchors.** The driver is the in-ear example of C. Poldy, *Headphone
Fundamentals*, AES 120th Convention tutorial (Paris, 2006), Fig. 20:
mechanical resonance 255 Hz, moving mass 17.5 mg, Bl 0.46 T·m, coil
resistance 14.4 Ω, diaphragm area 0.8 cm², rear cavity 0.15 cm³. It is a
...
Qes = 2π·fs·Mms·Re/Bl² = 1.91. The rated impedance is 16 Ω (estimate). The
...
from 50 Hz to a third of the in-situ resonance (the impedance maximum near
1.3 kHz; tested). The relief vent lifts the level below its corner near
...
- **Corners.** Every `examples/design_*.json` solves with finite probe
  ...
  bound, 64 random corners, and the densest grid: 626 solves over the three
  templates, at 6 points per octave from 10 Hz to 20 kHz (96 for the densest
  grid).
...
- **Closed forms.** Each template against a lumped network written out in
  the test: the driver from its primary set; adiabatic compliances with the
  first-order thermal wall layer; slits (tanh form) and tubes (Bessel form,
  by power series) as incompressible thermoviscous ducts; textbook end
  corrections and radiation loads; meshes as R_s/A; the ear's input impedance
  from the engine. The front (or canal-entrance) pressure agrees within
...
- C. Poldy, *Headphone Fundamentals*, tutorial, AES 120th Convention, Paris,
  2006 (Philips Sound Solutions), Fig. 20.
